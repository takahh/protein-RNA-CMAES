...
#  take a list of id and returns training and test set for the fold number
def get_five_folds(id_list, foldnum):
    testsize = int(len(id_list) / 28) + 1
    if foldnum == 4:
        test_list = id_list[4 * testsize:]
    else:
        test_list = id_list[int(foldnum) * testsize:(int(foldnum) + 1) * testsize]
    train_list = [x for x in id_list if x not in test_list]
    return [test_list, train_list]


# read a file, foldnum, and cpu id and returns a list of id
def read_train_id(fold_num, j, input_path):  # j: cpu num
    df = pd.read_csv(input_path)
    id_list_all = df.values.tolist()
    training_list = get_five_folds(id_list_all, fold_num)[1]
    # decide batch list
    batchsize = int(len(training_list) / 28) + 1
    if j == 28 - 1:
        batch_list = training_list[j * batchsize:]
    else:
        batch_list = training_list[j * batchsize:(j + 1) * batchsize]
    return batch_list
...
def calc_dots(arg_list):  # init_vec, fold_num, input_path
    ...
    init_vec = arg_list[0]
    input_path = arg_list[2]
    train_list = remove_remove_id2(read_train_id(i, j, input_path))
...
def get_avg(init_vec, i, logfile, inputpath):  # i : fold number
    print(f'{i}*************')
    import datetime
    ...
    init_vec_repeat = []
    for k in range(np):
        init_vec_repeat.append([init_vec, i, inputpath])
...
def main(inp_path, o_dir, subset_n, i, previous_log_file):
    import cma
    sd0 = 0.5
    log_file = f'{o_dir}cmaes_log_{i}_subset{subset_n}_{current_date}.txt'
    ...
    if use_previous_result:
        ...
        init_pot = [0.01] * 152
    cma.CMAEvolutionStrategy(init_pot, sd0).optimize(get_avg, iterations=900, args=[i, log_file, inp_path])
...
if __name__ == "__main__":
    ...
    fold_num = sys.argv[4]
    pre_result_path = sys.argv[5]
    main(input_path, out_dir, subset_num, fold_num, pre_result_path)
